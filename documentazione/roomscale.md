...
| 2026-06-10 | Debug visivo dei confini stanza: `EffectMesh.HideMesh` impostato a `false` — la geometria ricostruita (pareti, pavimento, soffitto, volumi mobili) è ora renderizzata in-app con `RoomBoxEffects.mat`, replicando la visualizzazione di Space Setup. Verificato su Quest 3S. Per tornare ai soli collider invisibili: rimettere `HideMesh = true`. Commit `3bd0909`. |
| 2026-06-10 | Fallback stanza demo per editor/simulatore: in editor `LoadSceneFromDevice` non trova stanze (nessuno Scene Model) e MRUK restava vuoto. `SceneAccessManager` ora, **solo in editor** (`#if UNITY_EDITOR`), se dopo il load da device `Rooms.Count == 0` carica il prefab assegnato al nuovo campo `editorFallbackRoomPrefab` (impostato a `Bedroom00` del package MRUK) via `LoadSceneFromPrefab`; EffectMesh genera la stanza con la stessa pipeline del device. Il codice device è invariato. Verificato nel simulatore. |
| 2026-06-10 | Comportamento palla all'avvio: nuovo script `GiveKinematics` sulla `Sphere`. Il `Rigidbody` è kinematic dall'`Awake`, quindi la palla resta sospesa a mezz'aria ad altezza utente; al primo grab (evento `PointerEventType.Select` del `Grabbable` ISDK) la fisica si attiva (`isKinematic = false`) e da lì gravità, lancio (`_throwWhenUnselected`) e rimbalzi funzionano normalmente. Elimina ogni dipendenza dal timing dei collider stanza: la palla non può cadere prima che l'utente la tocchi. |
| 2026-06-10 | Spawn palla relativo all'utente: `GiveKinematics` ora posiziona la palla all'avvio davanti alla testa dell'utente — altezza occhi, `spawnDistance` (default **0.3 m**, regolabile da Inspector) lungo la direzione di sguardo proiettata sull'orizzontale. Aspetta che il tracking della testa dia una posa valida (timeout 2 s) prima di posizionarla. |
